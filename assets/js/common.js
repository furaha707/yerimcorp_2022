  ...
  gnbdrop_all: function () {
    ...
    // $header = $('.header'),
      ...
      $('.header .nav').each(function () {
        ...
        $(this).find('.nav-list--depth2').each(function () {
...
var subNav = {
  ...
  depth_clone: function () {
    ...
    var $depth2List = $('.sub-nav-clone--depth2 .container'); //depth2Active를 복사해 넣을 컨테이너
    var $depth3List = $('.sub-nav-clone--depth3 .container'); //depth3Active를 복사해 넣을 컨테이너
...
var subNavScroll = {
  ...
  sticky_nav: function () {
    var $subNav = $('.sub-nav--bar'),
      $subNavWrap = $('.sub-nav--bar .sub-nav__wrap'),
      $headerHeight = $('.header').outerHeight();

    if ($subNav.length > 0) {
      var subNavOffset = $subNav.offset(),
        subNavTop = subNavOffset.top - $headerHeight; // #헤더 고정형일때


      // if ($(window).outerWidth() > 1024) {
        if ($(window).scrollTop() > subNavTop) {
          $subNav.addClass('fix');
          $subNavWrap.css('top', $headerHeight); //#헤더 고정형일때
  ...
  //        $subNavHeight = $('.sub-nav.fix').find('.sub-nav__wrap').height(),
  //         $headerHeight = $('.header').outerHeight();
...
$(document).ready(function(){
  var $nav_drop = $('.header .nav .nav-list--depth2'),
        $nav_link = $('.nav .depth-1').children('.link'),
        $nav = $('.nav'),
        $header = $('.header'),
        $nav_bg = $('.nav__bg'),
        $nav_Btn = $('.nav-pc__btn');

  $nav.addClass('gnb-drop--each');        

  $nav_link.on('mouseover focus', function () {
    if ($nav_Btn.hasClass('is-active') == false) {
      $nav_drop.stop(false, true).slideUp(500,'easeOutQuart');
      $(this).next().stop(false, true).slideDown(500,'easeOutQuart');
    }
  });
  $header.on('mouseleave', function () {
    if ($nav_Btn.hasClass('is-active') == false) {
  ...
  /* 헤더 전체 메뉴 */
  $('.nav-pc__btn').on('click', function(){
    $(this).stop().toggleClass('is-active')//slideDown(200);

    if($('.nav-pc__btn').hasClass('is-active')){
      $nav_bg.slideDown(500,'easeOutQuart'); 
      $nav_drop.slideDown(500,'easeOutQuart'); 
    }else{
      $nav_drop.slideUp(500,'easeOutQuart');
      $nav_bg.slideUp(500,'easeOutQuart');
    }
  });
})
...
//mypage 메뉴
function subNavScroll_front() {
  var $subNav = $('.mypage-nav .nav-list--depth2');
  var subNavLinkLeft = $('.mypage-nav .nav-list--depth2 .link.on').offset().left;
  var subNavWidth = $('.mypage-nav .nav-list--depth2');
  var totalWidth = 0;
...
$(document).ready(function () {
  if ($('.mypage-nav .nav-list--depth2').length > 0) {
    if ($(window).innerWidth() < 1201) {
      subNavScroll_front();
    }
  }
...
// 푸터 패밀리사이트 열고 닫기
$(document).ready(function () {
  $(".footer-familysite__btn").click(function () {
    $('.footer-familysite__list').slideToggle(300);
  });
  $(".footer-familysite__list .link").click(function () {
    $('.footer-familysite__list').slideUp(300);
  });

  $('.link-on').removeClass('on');
  $('.link-on').removeClass('active');
